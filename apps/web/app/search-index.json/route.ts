...
// it can run once during `next build` and be served as a prerendered file
// from then on, exactly like a page under `output: "export"`. `/blocks`
...
// it is a filesystem read baked into the deploy — so prerendering is the
// correct choice here, not a shortcut taken for the same reason `/blocks`
// cannot.
