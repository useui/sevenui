  ...
  return (
    <header className="sticky top-0 z-40 flex h-16 items-center gap-2 border-b sm:gap-3 bg-background/90 px-4 backdrop-blur md:px-6">
      ...
      <nav aria-label="Sections" className="hidden gap-1 lg:flex">
        {tabs.map((tab) => (
          <Link
            ...
            className="shrink-0 whitespace-nowrap rounded-full px-2 py-1.5 xl:px-3 font-medium text-muted-foreground text-sm transition-colors hover:text-foreground aria-[current=page]:text-foreground"
            href={tab.href}
            key={tab.href}
          >
            {tab.label}
      ...
      <div className="flex shrink-0 items-center gap-2">
        ...
        >
          <span
            className="hidden group-data-[signed-in]:max-sm:inline"
            data-auth-initial
          >
