...
The bar is **behaviour + layout parity**. Information architecture, routes, interactions and overall visual layout must match; a few px of drift, font-rendering differences and spacing rounding are acceptable. `/` and `/blocks` are held closer to pixel parity — both were hand-tuned in dedicated efforts. **That closer bar holds in light and has one declared exception in dark (§17.6 #48):** the five aliased `--color-*` tokens take the design system's own dark values, one of which inverts the border from dark-grey-80% to white-10%, and it lands on exactly these two surfaces. The bar is not withdrawn — the exception is one row, measured and accepted — but a reader checking the standard reads the standard, so it is stated here rather than only where it is declared.
...
| 48 | **The dark theme's five aliased `--color-*` tokens take the design system's own values, and `border` inverts.** `globals.css` aliases `--color-background`, `--color-foreground`, `--color-border`, `--color-muted` and `--color-muted-foreground` to the preset's variables — all five are read by demos — and in **dark** all five differ from Blume's. Measured through a browser on both live origins, 2026-09-21: canvas `oklch(0.085 0 0)` = rgb(2,2,2) → rgb(10,10,10); body text `oklch(0.96 0 0)` → `lab(98.26 0 0)`; and **`border` `oklch(0.24 0 0 / 0.8)`, dark grey at 80%, → `lab(100 0 0 / 0.1)`, white at 10% — an inversion, not a shift.** **Light mode is byte-identical.** It reaches `/` and `/blocks`, the two surfaces §17.3 holds near pixel parity, which is why it is declared rather than absorbed. Accepted deliberately: #5 forbids coining a replacement vocabulary, and a white-10% border on a near-black canvas is the design system's own idiom rather than a defect in it. **This is not #14 widened** — #14 is one token, the focus ring, bounded at ≤0.025 L; a 0.06 L canvas move and an inverted border are outside that bound in size and in kind, and folding them in would retire the bound instead of stating the change | §8.3, §17.3 |
