...
| 8 | **Dark-mode chrome background — A DECISION, NOT A TICK** | Production's chrome paints `--color-background` from Blume's own `--blume-background` = `oklch(8.5% 0 0)` = **rgb(2,2,2)**; the port collapses it onto the design system's `--background` = `oklch(0.145 0 0)` = **rgb(10,10,10)**. Measured in the captured PNG pixels on the Stage 5 preview; 15-17 opaque elements per page are painted with it, on **every route**, dark mode only. Light mode is identical and the other 16 theme tokens match exactly in both themes. **Nobody chose this** - it is not a §17.6 row, it is a miss no gate in this plan can see (the extractor is style-blind by design, and every pixel matrix measured geometry rather than colour). Two options: restore production's value for the chrome only, leaving `@sevenui/presets` to own `--background` inside preset scopes as it does today; or accept that the site's chrome now matches the design system it publishes, and say so. | Someone with the site in front of them, in dark mode, on both origins |
